...
(defn parse-tag
  ...
  (let [tag-str (name tag)]
    (when (and (not (re-matches re-tag tag-str))
      ...
      (throw (#?(:clj NullPointerException. :cljs js/Error.) (str "Invalid tag name (found: " tag-str "). Make sure that the name matches the format and ordering is correct `:tag#id.class`"))))
    (let [[tag id class-name] (next (re-matches re-tag tag-str))
          tag (if (= "" tag) "div" tag)
          class-name (when-not (nil? class-name)
                       (str/replace class-name #"\." " "))]
      #?(:clj (list tag id class-name (some? (re-find #"-" tag)))
         :cljs (cljs.core/array tag id class-name (some? (re-find #"-" tag)))))))
